...
* Dark mode preference is persisted in local storage when the report is served over HTTP; when opened as a local file, the OS theme is used by default and the toggle is kept for the browser session via URL state
